...
# --------------------------------------------------------------------------
# --------------------------------------------------------------------------
# 5. THE STARK QUANTUM PROTOCOL (Absolute Zero & Logic Gates)
# --------------------------------------------------------------------------
...
print("\nDeploying The Stark QUANTUM Protocol (v8)...")

# Stark Constants (Agressive Defensive Posture)
HARD_CEILING = 4800.0  # Training Invariant (Max hit ever recorded was 4.67km)
V_MAX_DEFENSIVE = 360.0 # m/h (Tighter than v7 to reduce noise)
Q_LOW = 0.15           # "Little Probability" Filter
Q_HIGH = 0.85          # Certainty Booster
...
for h, col in zip(horizons, cols):
    # 1. THE EVENT HORIZON: Absolute boundary check
    mask_ceiling = test_dist > HARD_CEILING
    submission.loc[mask_ceiling, col] = 0.0
    
    # 2. DEFENSIVE V_MAX: Internal unreachable zone zeroing
    # Scaled by seasonal hazard
    v_max_h = V_MAX_DEFENSIVE * (seasonal_hazard * 0.8 + 0.2)
    limit = v_max_h * h * 1.10 # 10% safety margin
    mask_internal_zero = (test_dist <= HARD_CEILING) & (test_dist > limit)
    submission.loc[mask_internal_zero, col] = 0.0
    
    # 3. QUANTUM CLIPPING: Killing the "Little Probabilities"
    # Low clip
    mask_low = (submission[col] > 0) & (submission[col] < Q_LOW)
    submission.loc[mask_low, col] = 0.0
    
    # High clip
    mask_high = (submission[col] > Q_HIGH)
    submission.loc[mask_high, col] = 1.0 # Force 100% certainty if model > 0.85
    
    # 4. CERTAIN STRIKE BOOSTER (Reinforced)
    mask_strike = (test_dist < 400) & (closing_speed > 2)
    submission.loc[mask_strike, col] = 1.0

    zero_counts[h] = (submission[col] == 0.0).sum()

print(f"  Final Zeroed rows: {zero_counts}")
...
out_path = os.path.join(BASE_DIR, 'wids_ultimate_v8_quantum_submission.csv')
submission.to_csv(out_path, index=False)

print(f"\nSUCCESS! Quantum v8 submission saved to: {out_path}")
