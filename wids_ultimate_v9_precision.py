...
# --------------------------------------------------------------------------
# --------------------------------------------------------------------------
# 5. THE STARK PRECISION PROTOCOL (v9 - Fine-Tuned)
# --------------------------------------------------------------------------
...
print("\nDeploying The Stark PRECISION Protocol (v9)...")

# Stark Constants (Balanced defensive posture)
HARD_CEILING = 5200.0  # Safe boundary from v6 (Prevents Row 31 regression)
V_MAX_DEFENSIVE = 360.0 # m/h 
V_MAX_MARGIN = 1.50    # 50% Safety margin to avoid accidental zeroing 
Q_LOW = 0.05           # Only kill the ultra-low probabilities
Q_HIGH = 0.95          # Only max out the ultra-high certainties
...
for h, col in zip(horizons, cols):
    # 1. THE EVENT HORIZON: Global boundary check
    ...
    submission.loc[mask_high, col] = 1.0 
...
out_path = os.path.join(BASE_DIR, 'wids_ultimate_v9_precision_submission.csv')
submission.to_csv(out_path, index=False)

print(f"\nSUCCESS! Precision v9 submission saved to: {out_path}")
